...
function getMinTimes(options) {
  return function(steps, step) {
    ...
    steps.push({
      ...
      }, mergeOptions(options, step))), {})
    });
    return steps;
  };
}

function mergeOptions(mainOptions, stepOptions) {
  return {
    distinctId: stepOptions.distinctId || mainOptions.distinctId,
    time: stepOptions.time || mainOptions.time
  };
}
